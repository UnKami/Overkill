...
## Encounter integration preview — 2026-09-19

Added a dedicated Custodian actor and opt-in real combat entry: `res://scenes/custodian_encounter.tscn -- --custodian-3d`. Uses the existing Act II elite data (75 HP, bulwark clock). Normal release routing is unchanged. Custodian has its own 14/30-second attack contact threshold, 1.2-second attack, held guard at 9/30 seconds, hit interruption, 1.6-second held shutdown and private visor fade. The arena routes guard/contact effects and terminal delays through this actor.

This is a playable integration preview, not final art or contact acceptance. It currently shares Boneghoul staging coordinates; battle captures are readable, but exact hand-to-body contact and Custodian-specific staging require measurement/refinement. The claw contact method is a skeletal landmark proxy, not a mesh collision result. Published installer remains 0.23 and does not contain this preview.

Reproducible regression: `Godot --path . --scene res://scenes/custodian_encounter_test.tscn -- --custodian-3d`. Tests actor contact/interruption/held guard/terminal state/private visor, then actual controller guard, partial block, HP damage and win/loss handoff in eight fast/reduced-motion combinations. Rendered captures go to `user://custodian-encounter/`. Use an isolated APPDATA profile to avoid disturbing a saved run.

Verification: `.tools/087-encounter.log` and `087-final.log` pass all eight real-controller terminal combinations. Actor-specific checks pass; initial creation/free-in-one-frame fixture produced null-material warnings, resolved by waiting for the first render before exercising/freeing actors. Added two-frame teardown wait; `087-actor-clean-final.log` verbose rerun has no renderer or ObjectDB leak warnings. Known certificate-store warning remains. Sentinel regression (`087-sentinel-regression.log`) passes original mesh, repeated strikes, contact, reduced motion, cleanup and framing. Its incidental timings are not a controlled performance result. Choice/impact/victory captures reviewed; exact physical contact remains unverified.
