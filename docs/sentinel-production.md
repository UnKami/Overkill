...
## GPU contention invalidates current optimization comparisons (2026-09-19)

Investigated variable frame pacing with sequential high/performance/high rendering, full/frozen-stage/full, and full/no-directional-shadow/full diagnostic fixtures. Full stage rendering dominated wall time; however baselines drifted heavily between tests. Do NOT use these timings to rank optimizations, assign a regression to the new model, or claim stable FPS.

Authoritative Windows GPU Engine counters sampled AFTER all test Godot processes exited showed SlayTheSpire2 (PID 51480) at 69.48% 3D engine utilization, a separate Overkill process (PID 52320) at 18.66%, and ChatGPT at 7.58%. Process names independently resolved with Get-Process; no Godot test process remained. Existing user games were not stopped. This proves substantial competing GPU workload at the observation, not that exactly these percentages persisted through every earlier test. Prior timing comparisons in sections 060/064 also lack an external-workload record and should not be treated as controlled performance acceptance.

Evidence `.tools/065-quality-final*`, `065-isolate*`, `065-shadows*`; standalone diagnostic's initial global-autoload identifier compile error corrected to root.get_node before measurements. Final runtime logs clean except certificate warning. High/performance/high stage GPU medians 16.654/12.464/19.303 ms; frozen-stage root GPU remained 1.944/2.231/3.098 ms while wall medians 31.796/10.881/43.080 ms. Shadow comparison drifted to 66.856/28.283/45.622 ms stage GPU. These are diagnostic observations under contention, not acceptance metrics. No rendering settings or production code changed. Next performance run requires competing games closed, an external GPU workload snapshot, and fresh matched baselines. Asked user to close them when convenient; art work can continue independently.

User follow-up: separately running Overkill closed; Slay the Spire 2 intentionally remains open. Continue art work, defer controlled performance acceptance.

## Sentinel dished knee armor (after 0.22; unreleased)

Replaced flat extruded knee badges with closed, faceted cups: outer rim, raised intermediate ring and projecting center, retaining the existing shin attachment. Final 39,426 vertices / four surfaces (+174 vertices). Blender build/import and rendered idle, guard, recoil and final kneeling-death close-ups checked; cup follows the shin and remains seated over the pivot in sampled poses. No skeleton/animation/material change. This is shape refinement, not full animation collision or AAA acceptance.

Evidence `.tools/066-build*`, `066-import.log`, `066-knee*`, `066-poses-final*`. Temporary pose fixture first requested nonexistent clip 'collapse'; assertion rejected that run, corrected to actual 'death', and the stalled test process was identified by command line and terminated. Final fixture captured all three poses and exited cleanly, with no script/assertion/shader errors; certificate warning remains. Performance profiling intentionally deferred while user plays another GPU-heavy game. Published 0.22 unchanged.

## Cloak folds and tailored binding (after 0.22; unreleased)

Increased transverse cloak tessellation from 16 to 48 columns (four to twelve samples per authored fold); retained 22 vertical rows. Indexed the mesh after generating normals: each cloak is 1,127 vertices / 6,336 indices / 2,112 triangles versus the prior 2,112 unindexed vertices / 704 triangles. More triangles, fewer stored unique vertices; no performance claim. Replaced the broad bright border with a narrow, darker woven binding, analytically antialiased with fwidth. Existing gravity-directed drape, ground constraint and motion parameters unchanged.

Rendered close-up back views of both actors reviewed in Vulkan and OpenGL; smoother hem silhouette and subdued edging retained. Sentinel production regression passes strikes, reactions, reduced motion and framing. `.tools/067-back*`, `067-production*`, `067-gl*` logs have no script/assertion/shader failures, known certificate warning remains. Named TailoredCloak for inspection. Performance intentionally not evaluated while user plays Slay the Spire 2. Cloth still lacks physical collision/simulation and bespoke wear; this is limited visual refinement, not AAA acceptance. Published 0.22 unchanged.

## Cloak movement response (after 0.22; unreleased)

Added bounded world-space hem lag driven by chest movement. Measured anchor velocity targets a trailing offset with exponential smoothing; maximum 9 cm, weighted quadratically down the cloth so the shoulder seam remains fixed. Existing gravity and ground constraints apply after the offset. Motion stops cleanly at rest; reduced motion, first sample, teleports over 0.5 m and frame stalls over 150 ms clear the response. Both rigged actors share this behavior; no geometry or new material added.

Focused manual-animation fixture checks 30/60/120 Hz translation responses (-0.037472/-0.037472/-0.037473 m), settling under 0.1 mm, teleport reset, reduced-motion reset and bounded actual Sentinel attack response (peak 0.072495 m). Attack/windup/recovery frames captured; impact-frame back view inspected. Full rendered Sentinel production checks pass contact, reactions, reduced motion and framing. Evidence `.tools/068-cloak*`, `068-production*`; no script/assertion/shader failures, known certificate warning only. This validates sampled response and integration, not physically simulated cloth, continuous collision or overall cinematic acceptance. Performance profiling deferred under user's concurrent game workload. Published 0.22 unchanged.

## Content-sized relic choices (after 0.23; unreleased)

Battle cards now allocate description height from rendered content (78 px minimum) instead of a fixed 120 px box. Opening starter choices shrink from 232 to 190 design pixels high while keeping existing font and action-button sizes. Longer effects expand; title height is respected. Bottom-anchored action buttons stay aligned across a mixed row. Refit on theme changes supports live text-size changes. Explicit bind destination and harmless art inspection remain intact.

Rendered 1080p and 720p large-text assembly captures reviewed: the empty band above the buttons is removed and the characters retain clear space. Existing presentation regression checks every live relic in normal/large text for clipping and button overlap, plus horizontal layout, safe bounds, previews, inspection, explicit commit, locked replacements, empty reserve and ultrawide. `.tools/070-presentation*` passes with no script/assertion/shader errors; known certificate warning remains. This does not establish overall AAA presentation or performance. Published 0.23 unchanged.
