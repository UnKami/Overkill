...
current_ok: int
best_single_hit_ok_this_run: int      # drives SINGLE_HIT_OK excess-tier unlock checks - a plain
                                        # running max with no act-scoping, by design (see note below)
...
**Excess-tier thresholds are pure OK-magnitude tiers, not act-gated.** An unusually large Act 1 hit that clears the Act 3 threshold unlocks Act 3-tier Excess cards immediately — the per-act numbers in the balance doc are pacing expectations for a typically-played run, not hard requirements that the player be physically in that act. This is a deliberate decision (confirmed, not an oversight left over from before this field existed): it rewards exceptional early play rather than making the player wait out an act they've already out-performed, and it keeps this field's implementation exactly as simple as it looks above — a plain running max, no per-act bucketing or scoping logic anywhere near it.
